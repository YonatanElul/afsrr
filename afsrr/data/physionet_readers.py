...
class BasePhysioReader(abc.ABC):
    ...
    def __init__(
            self,
            db_path: str = None,
            data_extenstion: Optional[str] = 'dat',
        ...
        if data_extenstion is not None:
            self.files = sorted(glob.glob(os.path.join(db_path, f'*.{data_extenstion}')))

        else:
            assert isinstance(annotation_extension, str)
            self.files = sorted(glob.glob(os.path.join(db_path, f'*.{annotation_extension}')))
    ...
    def read_record(
        ...
        if self._data_extenstion is not None:
        ...
        if isinstance(self._annotation_extension, dict):
            anno = wfdb.rdann(
                self.files[index].rstrip(f".{self._annotation_extension['anno']}").rstrip(f".{self._data_extenstion}"),
            ...
            anno = wfdb.rdann(
                self.files[index].rstrip(f".{self._annotation_extension}").rstrip(f".{self._data_extenstion}"),
        ...
        if self._detect_qrs_locations and record is not None:
...
class NormalSinusRhythmRRReader(BasePhysioReader):
    ...
    def __init__(
        ...
        """
        Constructor method for the NormalSinusRhythmRRReader class
...
class AFDBReader(BasePhysioReader):
    """
    ...
    def __init__(
        ...
        """
        Constructor method for the AFDBReader class
...
class ThewReader(BasePhysioReader):
    def __init__(
            self,
            db_path: str,
            single_signals_duration: int = (15 * 60),
            cache_dir: Optional[str] = None,
            detect_qrs_locations: bool = True,
            qrs_detection_replacement_gap: float = 0.05,
    ):
        """
        :param db_path:
        ...
        super().__init__(
            ...
            verbose=False,
            cache_dir=cache_dir,
            detect_qrs_locations=True,
            qrs_detection_replacement_gap=qrs_detection_replacement_gap,
        )

        # Load the database
        self.db_path = db_path
        self.frequency = 200  # In [Hz], From PhysioNet

        self.records_lengths = []
        self.nsr_duration = []
        self.af_onsets = []
        self.af_durations = []
        self.other_onsets = []
        self.other_durations = []
        self.files_names = []
        self.single_signals_duration = single_signals_duration * self.frequency

    def read_record(self, index: int = 0) -> Dict:
        if self._verbose:
            print(f"Reading record #{index + 1}: {self.files[index]}")

        # Read record
        record_name = (self.files[index].split(os.sep)[-1]).split('.')[0]
        record = ishneholterlib.Holter(self.files[index])
        record.load_data()

        lead_1 = record.lead[0].data
        lead_2 = record.lead[1].data

        qrs_path = os.path.join(self._cache_dir, f"{record_name.split(os.sep)[-1]}_qrs.npy")
        if os.path.isfile(qrs_path):
            qrs_annotations = np.load(file=qrs_path, allow_pickle=True)

        else:
            n_splits = lead_1.shape[0] // self.single_signals_duration  # Potentially dropping the last partial window
            splitted_signal = [
                lead_1[(i * self.single_signals_duration):((i + 1) * self.single_signals_duration)]
            ...
            qrs_annotations = [
                ...
                for split in splitted_signal
            ]
            qrs_annotations = [
                qrs_annotations[i] + (i * self.single_signals_duration)
                for i in range(len(qrs_annotations))
            ]
            qrs_annotations = np.concatenate(qrs_annotations, 0).astype(int)

            # Cache the annotations
            np.save(file=qrs_path, arr=qrs_annotations)

        labels = np.zeros_like(qrs_annotations)
        leads = np.concatenate([lead_1[:, None], lead_2[:, None]], axis=1)
        comments = []
