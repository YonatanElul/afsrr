...
from afsrr.utils.losses import LossComponent
...
class Trainer(BaseTrainer):
    ...
    def train_batch(self, batch, ignore_cap: bool = False) -> Tuple[float, float]:
        ...
        if 'af_labels' in batch[OTHER_KEY]:
            af_labels = batch[OTHER_KEY]['af_labels'].to(self._device)
            outputs['af_labels'] = af_labels

        loss = self._loss_fn(outputs)

        # Run the backwards pass
        if isinstance(loss, int) and loss == 0:
            # Loss is actually NaN, so skip it
            self._optimizer.zero_grad()
            return 0, 0
